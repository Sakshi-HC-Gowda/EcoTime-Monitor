...
export function DashboardPage() {
  ...
  // Analytics is an enhancement, not a blocking dependency — the dashboard
  // still renders (with heuristic fallbacks) if this call is slow or fails.
  const { data: summary, isError: summaryError } = useDashboardSummary('US-CA');
  ...
  // EcoScore — prefer the real, history-backed analytics score; fall back to a
  // heuristic from current grid intensity if analytics hasn't loaded yet.
  const ecoScore = summary && !summaryError
    ? Math.round(summary.ecoScore)
    : Math.max(10, Math.min(99, Math.round(100 - (currentIntensity / 400) * 100)));

  const carbonSavedKg = summary?.totalCarbonSavedKg ?? 0;
  const carbonSavedLabel = carbonSavedKg >= 1
    ? `${carbonSavedKg.toFixed(2)}`
    : `${(carbonSavedKg * 1000).toFixed(0)}`;
  const carbonSavedUnit = carbonSavedKg >= 1 ? 'kg CO₂' : 'g CO₂';
  ...
  return (
    <div className="page-shell page-stack">
      ...
      <div className="card-grid card-grid-sm-2 card-grid-lg-4 items-stretch">
        ...
        <MetricCard
          title="Carbon Saved"
          value={carbonSavedLabel}
          unit={carbonSavedUnit}
          subtitle={summary ? 'Cumulative via accepted recommendations' : 'Loading cumulative savings…'}
          icon={Leaf}
              ...
              {summary?.latestRecommendation ? (
                <>
                  <p className="text-xs text-slate-300 leading-relaxed">
                    {summary.latestRecommendation.text}
                  </p>
                  ...
                  {summary.latestRecommendation.expectedCarbonSaving != null && (
                    <p className="text-[11px] text-green-400 font-semibold mt-1.5">
                      Est. saving: {Math.round(summary.latestRecommendation.expectedCarbonSaving)} g CO₂
                    </p>
                  )}
                </>
              ) : (
                <p className="text-xs text-slate-500 leading-relaxed">
                  No recommendations yet — run the Scheduler or check a task's EcoScore to generate one.
                </p>
              )}
            </div>
          </GlassCard>
