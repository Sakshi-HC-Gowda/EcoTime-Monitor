...
GET    /api/activities/<id>/workload — Get the activity's workload profile
"""
...
def bulk_update():
    ...
    updated_tasks, error = bulk_update_activities(updates)
    if error:
        return jsonify({
            "success": False,
            "error": error,
...
@activities_bp.route("/activities/<task_id>/workload", methods=["GET"])
def get_activity_workload(task_id: str):
    """
    Get the estimated workload profile for an activity
    (CPU usage, workload level, prediction score, workload category).

    Returns 200: { success: true, data: WorkloadProfile }
    Returns 404: { success: false, error: str }
    """
    profile = get_workload_profile(task_id)
    if not profile:
        return jsonify({
            "success": False,
            "error": f"No workload profile found for activity '{task_id}'",
            "timestamp": _now_iso(),
        }), 404
...
@activities_bp.route("/activities/<task_id>/current-status", methods=["GET"])
def get_activity_current_status(task_id: str):
    """
    Get the dedicated current-status record for an activity.

    Returns 200: { success: true, data: CurrentStatus }
    Returns 404: { success: false, error: str }
    """
    status_row = get_current_status(task_id)
    if not status_row:
        return jsonify({
            "success": False,
            "error": f"No current status found for activity '{task_id}'",
            "timestamp": _now_iso(),
        }), 404
