...
class Activity(db.Model):
    ...
    Status state machine:
        draft → pending → scheduled → waiting_for_device → running → completed
              ↘ idle (legacy)  ↘ delayed (legacy)  ↘ paused → running
                                                    ↘ cancelled / missed / failed (legacy)
    """
    ...
    category = db.Column(db.String(64), nullable=False, default="batch-processing", index=True)
    estimated_energy_kwh = db.Column(db.Float, nullable=True)
    estimated_carbon_grams = db.Column(db.Float, nullable=True)
    ...
    def to_dict(self) -> dict:
        ...
        """
        energy = self.estimated_energy_kwh
        if energy is None:
            energy = round((self.duration / 60.0) * (self.power_draw / 1000.0), 4)
        ...
        return {
            ...
            "estimatedEnergyConsumption": energy,
            "estimatedCarbonGrams": self.estimated_carbon_grams,
    ...
    def from_dict(cls, data: dict) -> "Activity":
        ...
        return cls(
            ...
            category=data.get("category", activity_type),
