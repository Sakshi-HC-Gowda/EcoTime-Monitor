...
def get_carbon():
    ...
    # Persist this reading so "carbon intensity" is an actual DB record,
    # not just an ephemeral response. Best-effort — never blocks the request.
    try:
        from services.analytics_service import log_carbon_snapshot
        current = data.get("current", {}) or {}
        if current.get("carbonIntensity") is not None:
            log_carbon_snapshot(
                region=zone,
                carbon_intensity=current["carbonIntensity"],
                forecast=data.get("forecast"),
                source="simulation" if data.get("isSimulated") else "live",
            )
    except Exception:
