...
export function ActivityCard({
  ...
  const recommendedDisplayTime =
    formatRecommendedTime(recommendedStartTime);

  const isHighCarbon = estimatedCarbonImpact > 200;

  const topBorderClass =
    statusAccent[activity.status.toLowerCase()] ??
    'border-t-white/[0.06]';

  const canAcceptRecommendation =
    activity.status === 'pending' &&
    recommendation.toLowerCase().includes('schedule');

  const isFileUpload = activity.activityType === 'file-upload';

  const progress =
    uploadState?.progress ?? activity.progress ?? 0;
    ...
    >
      ...
      <div className="flex items-start justify-between p-[var(--card-padding-md)] pb-5">
        <div className="min-w-0 pr-3">
          ...
          <h3 className="mt-0.5 text-[15px] font-bold leading-tight text-white">
            {activity.name}
          </h3>
        </div>
          ...
          <div className="rounded-xl border border-white/[0.07] bg-white/[0.03] p-3">
            ...
            {uploadIsComplete && uploadedFile && (
              <div className="mt-3 rounded-lg border border-green-500/20 bg-green-500/[0.05] p-3">
                <p className="mb-3 text-xs font-bold text-green-300">
                  ✓ Uploaded Successfully
                </p>
                ...
                <UploadMeta
                  label="File Size"
                  value={uploadedFile.sizeFormatted}
                />

                <UploadMeta
                  label="Uploaded At"
                  value={formatUploadTime(
                ...
                <UploadMeta
                  label="Storage"
                  value={uploadedFile.storagePath}
                />
              </div>
            )}
